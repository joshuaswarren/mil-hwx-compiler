...
static H16GEncodedTDProgram *encodeScale(NSError **error) {
    ...
    WRITE(0x0ac,0x3db50000,@"alu.measured_inverse_sqrt_128");
...
+ (H16GEncodedTDProgram *)encodeScalarScaleForMatrixRows:(NSUInteger)rows
    ...
    if (rows != 128 || columns != 128 ||
        scaleKind != H16GMeasuredScaleInverseSqrt128) {
        setError(error,@"scalar scale requires the measured 128x128 inverse-sqrt-128 row");
        return nil;
    }
    return encodeScale(error);
}
