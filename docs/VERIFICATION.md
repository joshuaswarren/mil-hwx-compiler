...
the CLI, the release-hygiene guard, and the production-route guard. The CLI compiled each fixture twice and
produced byte-identical output. It also compiled the mixed graph from `/tmp`
using absolute input and output paths, with no descriptor-resource tree in its
working directory.
...
|---|---|---|
| FP16 attention forward | S128/D128, unmasked | 8 artifacts; zero mismatches; maximum absolute error `9.92883e-06` |
| FP16 affine state scan | 4 N128 multiply-add transitions | every stage within 1 FP16 ULP; final result within 3 ULP |
| Matmul -> reshape -> GELU | N128 and N256 | matmul element-exact; GELU maximum absolute error `0.00610352` |

The attention result covers one query tile and one key/value tile. The compiler
does not yet emit the S256 plan or causal masking. The affine scan validates
the recurrent state update used by chunked DeltaNet; it does not compile a
complete DeltaNet block.
