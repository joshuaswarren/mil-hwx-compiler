...
The committed records decode 4,928 tasks: 2,677 on H13 and 2,251 on H14, of which 1,790 and 1,736 come from the envelope campaign. **Evidence: high; recounted over `git ls-files research/oracles`.**

Both campaigns were minted on `studio-host.local`, an M1 Ultra running macOS 26.6.2, with one Apple compiler build whose SHA-256 every record carries, alongside the hashes of the minting driver and the decoder. Each record states its own provenance, because the first campaign was deliberately not re-minted when the envelope campaign was added. **Evidence: high for the recorded provenance.** See [oracle-diff.md](../../research/oracle-diff.md) and [oracle-envelope.md](../../research/oracle-envelope.md).
...
| Chains | `x + relu(x)` folds to one task; `gelu`/`silu` residuals cost two because they need their table; `matmul → gelu → matmul` costs one task more than a lone matmul at the same M; `matmul → softmax → matmul` stays one program at 9 to 11 tasks | [oracle-envelope.md](../../research/oracle-envelope.md) §5, `env_chain_residual_relu_1x768x1x1`, `env_chain_mlp_m32_k2048_h4096`, `env_chain_attention_s256_d64` |

Convolution constant sections take three shapes. A stride-1, groups-1 convolution stores exactly the weights; a bias adds a fixed 1,024 bytes for 64 or 256 output channels and 2,048 for 768, not `outputs * 2`; grouping and stride 2 each add material beyond the weights, and where either is in play the `bias0` and `bias1` sections are the same size, so that material absorbs the bias. The added bytes are measured, not explained. **Evidence: high for the sizes; open question for their content.** See section 3 of [oracle-envelope.md](../../research/oracle-envelope.md).
...
- **Open question:** What is in a convolution's constant section beyond the weights when grouping or stride 2 is used, and why is a bias a fixed 1,024 or 2,048 bytes rather than `outputs * 2`?
