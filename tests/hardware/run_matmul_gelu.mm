...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if (!runtime || bundle.artifacts.count != 1 || inputs.count != 2 ||
            outputs.count != 1 || !output)
            return 2;
            ...
            for (NSUInteger run = 1; valid && run <= 2; ++run) {
                ...
                if (!evaluated)
                    fprintf(stderr, "evaluate: %s\n",
                            error.description.UTF8String);
                valid = evaluated && validate(output, run, size);
            }
        } @finally {
