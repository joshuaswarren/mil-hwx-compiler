...
`H13_DEADLINE_SECONDS=120 bash tests/run_h13_linux_hardware.sh [--package DIR] MIL MODEL_ROOT NAME=input.fp16 ...`
...
The runtime loads the selected `--libane-library` and requires this ABI before transfer: `pyane_init`, `pyane_free`, `__ane_src_size`, `__ane_dst_size`, `__ane_send`, `__ane_read`, and `ane_exec`. Each program's returned surface sizes are validated before that program's first transfer. Package structure, dense fp16 input sizes, MIL return names and reference outputs are checked before dispatch; output files are written only after every output meets its numerical criterion. Device execution also requires a positive finite `H13_DEADLINE_SECONDS` whole-device-phase budget. The runner checks it before each program and never attempts to interrupt or recover an in-flight submission.

## Native-only benchmarks

`tools/h13_run_linux.py` offers `--benchmark-json PATH`, optionally `--warmup N` and `--iterations N`, for native execution only. These options are rejected with `--dry-run`; `--warmup` and `--iterations` require `--benchmark-json`. Defaults are three validated warmups and ten measured iterations. Native benchmarks retain the required `--deadline-seconds` whole-device-phase budget.
