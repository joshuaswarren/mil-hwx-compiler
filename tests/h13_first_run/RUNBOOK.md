# H13 first-run kit — the first ANE execution on m1-test-host
...
readiness is not permission to submit: the hardware handoff for m1-test-host is the
...
State on the workstation used to build this kit, for contrast with what m1-test-host
...
rejected. m1-test-host's checkout must be on `omarchy` (`f8df7bc` has the `0x1000`
...
* After a reboot, bring the engine up with
  `~/src/homelab-infra/scripts/m1-test-host-ane-bringup.sh` (it re-powers the chain
...
PREFLIGHT FAIL ane module is not loaded; run the m1-test-host bring-up ladder first
PREFLIGHT FAIL no character device at /dev/accel/accel0
PREFLIGHT FAIL $HOME/src/omarchy-ane is on 'omarchy-kmd', not 'omarchy'
